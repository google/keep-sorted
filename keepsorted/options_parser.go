...
func (p *parser) popListOfRegexpTemplatePair() ([]*regexpTemplatePair, error) {
	if p.allowYAMLLists {
		...
		if err == nil {
			...
			for _, item := range items {
				...
				case map[string]any:
					if len(v) != 1 {
						return nil, fmt.Errorf("pLORTP many whelps: %v", v)
					}
					for s, t := range v {
						if ts, ok := t.(string); ok {
							...
							return nil, fmt.Errorf("pLORTP unexpected type for map: %v", v)
						}
						break
					}
				default:
	...
	for _, item := range strings.Split(val, ",") {
		rv = append(rv, &regexpTemplatePair{Regexp: regexp.MustCompile(item)})
	}
	return rv, nil
}
